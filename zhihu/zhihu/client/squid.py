...
from zhihu.config.rules import (
    SCORE_MAPS, TTL_MAPS,
    SPEED_MAPS)
from zhihu.utils import (
    get_redis_conn, decode_all)
from zhihu.config.settings import (
...
class SquidClient:
    ...
    def __init__(self, task):
        if task not in SCORE_MAPS.keys():
            logger.warning('task value is invalid, https task will be used')
            task = 'https'
        ...
        if not SQUID_BIN_PATH:
            try:
                ...
                logger.warning('no squid is installed on this machine, or the installed dir is not '
    ...
    def update_conf(self):
        ...
        with open(self.template_path, 'r') as fr, open(self.conf_path, 'w') as fw:
            ...
            if not proxies:
                ...
                logger.info('no proxies got at this turn')
            else:
        ...
        logger.info('update squid conf successfully')
